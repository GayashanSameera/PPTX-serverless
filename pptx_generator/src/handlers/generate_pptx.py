...
def generatePptx(event, context):
   
    prs = Presentation('task-sample.pptx')
    ...
    class CommandRegistry:
        ...
        def get_command(self, command_name):
            ...
            if command_name == Command.REPLACE_IMAGE:
                image = Image()
                return image.replace_images(CommandRegex.IMAGE)
            else:
    ...
    class CommandExecutor:
        ...
        def __init__(self, presentation, replacements):
            self.slide = None
        ...
        def execute(self):
            ...
            for shape in self.slide.shapes:
                if shape.has_text_frame:
                    if shape.text in commands:
                        self.registry.get_command(Command.IF_CONDITION)(commands_dic, self.presentation, self.slide, shape, slides.index(self.slide), self.replacements)
